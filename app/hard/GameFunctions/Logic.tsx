...
const WordGuessingGame: FC<WordGuessingGameProps> = ({ WORDS, storyId, stylesOverride,congratulationsText }) => {
  ...
  const getUnplayedWords = async (storyId: string): Promise<Word[]> => {
    try {
      const completedHard = await fetchCompletedGames(storyId);
      if (!Array.isArray(completedHard)) {
          console.error(`Completed games for story ${storyId} is not an array:`, completedHard);
          return WORDS; // Return all words if there is an error
      }
      return WORDS.filter((_, index) => !completedHard.includes(index));
    } catch (error) {
  ...
  const saveCompletedGames = async (storyId: string, completedHard: number[]) => {
    try {
      console.log(`Saving for storyId: ${storyId}, data: ${JSON.stringify(completedHard)}`); // Debugging
      await createKvStore.setItem(`completedHard_${storyId}`, JSON.stringify(completedHard));
      console.log(`Saved completed games for story ${storyId}:`, completedHard);
    } catch (error) {
  ...
  const updateCompletedGames = async (storyId: string, completedIndex: number) => {
    try {
      const completedHard = await fetchCompletedGames(storyId);
      if (!completedHard.includes(completedIndex)) {
          const updatedGames = [...completedHard, completedIndex];
  ...
  const fetchCompletedGames = async (storyId: string): Promise<number[]> => {
    try {
      const savedData = await createKvStore.getItem(`completedHard_${storyId}`);
      const completedHard = savedData ? JSON.parse(savedData) : [];
      console.log(`Fetched completed games for story ${storyId}:`, completedHard); // Debugging

      if (!Array.isArray(completedHard)) {
        console.error(`Fetched completed games for story ${storyId} is not an array:`, completedHard);
        return [];
      }
