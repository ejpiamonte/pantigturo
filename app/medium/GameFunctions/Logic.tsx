...
const WordGuessingGame: FC<WordGuessingGameProps> = ({ WORDS, storyId, stylesOverride,congratulationsText }) => {
  ...
  const getUnplayedWords = async (storyId: string): Promise<Word[]> => {
    try {
      const completedMedium = await fetchCompletedGames(storyId);
      if (!Array.isArray(completedMedium)) {
          console.error(`Completed games for story ${storyId} is not an array:`, completedMedium);
          return WORDS; // Return all words if there is an error
      }
      return WORDS.filter((_, index) => !completedMedium.includes(index));
    } catch (error) {
  ...
  const saveCompletedGames = async (storyId: string, completedMedium: number[]) => {
    try {
      console.log(`Saving for storyId: ${storyId}, data: ${JSON.stringify(completedMedium)}`); // Debugging
      await createKvStore.setItem(`completedMedium_${storyId}`, JSON.stringify(completedMedium));
      console.log(`Saved completed games for story ${storyId}:`, completedMedium);
    } catch (error) {
  ...
  const updateCompletedGames = async (storyId: string, completedIndex: number) => {
    try {
      const completedMedium = await fetchCompletedGames(storyId);
      if (!completedMedium.includes(completedIndex)) {
          const updatedGames = [...completedMedium, completedIndex];
  ...
  const fetchCompletedGames = async (storyId: string): Promise<number[]> => {
    try {
      const savedData = await createKvStore.getItem(`completedMedium_${storyId}`);
      const completedMedium = savedData ? JSON.parse(savedData) : [];
      console.log(`Fetched completed games for story ${storyId}:`, completedMedium); // Debugging

      if (!Array.isArray(completedMedium)) {
        console.error(`Fetched completed games for story ${storyId} is not an array:`, completedMedium);
        return [];
      }
...
const styles = StyleSheet.create({
  ...
  congratulations: {
    ...
    alignItems: 'center',
    textAlign: 'center',
    textShadowColor: '#ffff00', // Outline color
  ...
  nextButton: {
    padding: 10,
    backgroundColor: 'rgba(255, 249, 191, .8)',
    borderRadius: 10,
    borderWidth: 2,
    borderColor: 'white',
    marginTop: 10,
  ...
  goBackButtonText: {
    fontSize: 24,
    color: '#ffffff',
    textShadowColor: 'grey', // Outline color
    textShadowOffset: { width: -1, height: 1 },
    textShadowRadius: 5,
    fontFamily: 'Inika-Regular',
  },
